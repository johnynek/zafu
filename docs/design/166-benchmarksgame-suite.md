...
6. Output handling: capture stdout directly for the four text benchmarks. For `mandelbrot`, keep the `bosatsu_jvm` target on the same `eval --run` path but redirect its stdout to a harness-owned temporary PBM file, treat that file as the source of truth, record byte count and SHA-256 from it, validate sample `N=200` against `fixtures/benchmarksgame/mandelbrot/mandelbrot-output-n200.pbm` with exact byte compare, perform no newline conversion, text decoding, or other normalization, and delete the temporary file after each measured run. The `java`, `bosatsu_c`, and `c` `mandelbrot` runs should likewise validate from temporary-file stdout capture rather than any text-normalized path.
