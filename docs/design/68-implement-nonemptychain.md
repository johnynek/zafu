...
Full design doc content for issue #68, including architecture, phased implementation plan, acceptance criteria, risks, and rollout notes for adding Zafu/Collection/NonEmptyChain.
...
depends_on:
  - 57
estimated_size: M
generated_at: 2026-03-08T22:45:00Z
---
...
1. `Zafu/Collection/Chain` provides stack-safe folds, O(1) concat, and efficient prepend/append construction, but it is nullable.
...
3. Introducing new abstract typeclass modules.
...
8. `is_empty(items: NonEmptyChain[a]) -> Bool`
9. `get(items: NonEmptyChain[a], idx: Int) -> Option[a]`
10. `get_or(items: NonEmptyChain[a], idx: Int, on_missing: () -> a) -> a`
...
3. `sort(order: Order[a], items: NonEmptyChain[a]) -> NonEmptyChain[a]`
...
3. Implement fold and indexing APIs directly on `Chain` delegates (`foldl_Chain`, `foldr_Chain`, `index_Chain`, `get_or_Chain`) to preserve `Chain` semantics.
4. Implement List-parity wrappers (`set`, `sort`, `zip`, `filter`, `distinct_by_hash`) via `to_List` and `Zafu/Collection/List` helpers, then reconstruct with `from_non_empty_Chain_or`.
5. Endpoint accessors are constructor-aware: fast path on matching constructor, fallback through `uncons_left_Chain` or `uncons_right_Chain` when needed.
6. Keep the module fully additive and self-contained; no required changes to `Chain` representation.
...
3. `size`, `foldl`, `foldr`, `map`, `flat_map`, `filter`, `reverse`, `distinct_by_hash`: O(n).
4. `get`, `set`: O(n).
5. `zip`: O(min(n, m)).
6. `sort`: O(n log n).
...
2. Add `singleton`, `from_prepend`, `from_append`, `from_Chain`, `from_List`, `to_Chain`, `to_List`.
3. Add core decomposition helpers (`head`, `tail`, `uncons`, `last`, `init`, `uncons_right`, `size`, `is_empty`).

Phase 2: parity API and composition

1. Add predicates and indexing wrappers (`any`, `exists`, `for_all`, `get`, `get_or`, `set`).
2. Add folds and transforms (`foldl`, `foldr`, `sum`, `sumf`, `map`, `flat_map`, `sort`, `zip`, `filter`, `distinct_by_hash`).
...
2. Add module tests mirroring `NonEmptyList` and `Chain` style: constructor pattern matching, conversion roundtrips, index edge cases, non-empty preservation, and ordering and hash sanity.
3. Validate with:
`./bosatsu lib check`
`./bosatsu lib test`
`scripts/test.sh`
...
4. Module includes conversions from and to both `Chain` and `List`.
5. `map`, `flat_map`, `sort`, and `concat` return `NonEmptyChain` and preserve non-empty guarantees.
...
8. `eq_NonEmptyChain`, `ord_NonEmptyChain`, and `hash_NonEmptyChain` are implemented and tested.
9. Tests cover negative and out-of-range indices and conversion roundtrips.
...
3. Risk: semantic drift from existing `List` and `Chain` behavior.
...
4. Follow-up optimization work can add stricter constructor normalization or cached endpoints if benchmarks show hotspots.
